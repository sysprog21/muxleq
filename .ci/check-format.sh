...
# Vendored guest code under tests/ and externals/ follows upstream style, not the
# project's, so it is excluded (except Forth: the project lints tests/*.fth too,
# matching the Makefile's FORTH_SRC). Formatter versions are enforced because
# output drifts between them. black and shfmt are checked to the EXACT release CI
# pins (pipx black==X, shfmt by content hash) so local == CI; clang-format is
# checked to major 20 only, since CI installs it from apt where the 20.x patch
# floats and no exact pin would match across environments.
set -euo pipefail

ret=0

# Collect NUL-delimited, git-tracked paths matching the pathspecs into the array
# `files` (empty if none). Avoids xargs' "run once on empty input" wart and works
# under bash 3.2 (macOS) where `mapfile -d` is unavailable.
collect() {
    files=()
...
# extended-regex pinned-version pattern $3; $4 is the human pin shown on a version
# miss. Exits on either. clang-format is checked below (it auto-detects the name).
require_fmt() {
