...
SCRATCH = (
    0x800  # RAM window offset for SW/LW round-trips; well above the tiny code+buffer
)
R_OPS = [
    (0, 0x00),
    (0, 0x20),
...
def build_program(rng, body_len):
    ...
    for r in WORK:  # prologue: seed every work register
        li(words, r, seeds[r])
    li(
        words, 30, SCRATCH, force_wide=True
    ...
    for _ in range(body_len):
        ...
        if kind == 5:  # forward computed/linking JALR, jumping over one ADDI
            ...
            desc.append(
                "JALR link x%d, target x%d = pc+16, skip [x%d = x%d + %03x]"
    ...
    for rd, rs1, rs2, f3, f7, extra in _expand_loops(ops):
        ...
        if f3 == "BR":  # branch (f7=funct3) guarding an ADDI rd = rs1 + imm
            ...
            if not branch_taken(f7, reg[b1], reg[b2]):  # taken -> skip the ADDI
                reg[rd] = alu(
                    0, 0x00, reg[rs1], sx32(extra if extra < 0x800 else extra - 0x1000)
                )
            continue
...
def run(cmd, stdin=None):
    try:
        return subprocess.run(
            cmd, capture_output=True, timeout=15, input=stdin, cwd=ROOT
        )
...
def main(argv):
    ...
    ap.add_argument(
        "--keep", action="store_true", help="save a failing .bin to the repo root"
    )
    ap.add_argument(
        "--wide",
        action="store_true",
