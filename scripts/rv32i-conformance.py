...
def gen_rtype(out):
    out.append(
        "\\ === R-type ALU: cross-product of edge operands (SLT/SLTU signed/unsigned) ==="
    )
    for name, f3, f7 in R_OPS:
        ...
        for a in CROSS:
            for b in CROSS:
                ...
                wlo, whi = lohi(w)
                out.append(
                    "%s %s %s %s %s %s %s %s chk  \\ %s a=%08X b=%08X"
...
def gen_shifts(out):
    out.append(
        "\\ === shifts SLL/SRL/SRA (R) and SLLI/SRLI/SRAI (I): every shamt 0..31 ==="
    )
    # R-type shifts: rs2 supplies the amount (and >32 must mask to low 5 bits)
    for name, f3, f7 in [("SLL", 1, 0x00), ("SRL", 5, 0x00), ("SRA", 5, 0x20)]:
        il, ih = lohi(enc_r(f7, f3))
        for a in SHIFT_VALS:
            for sh in list(range(32)) + [
                32,
                33,
                63,
                0xFF,
            ]:  # incl. amounts that must mask
                w = alu(f3, f7, a, sh)
                alo, ahi = lohi(a)
                blo, bhi = lohi(sh)
                wlo, whi = lohi(w)
                out.append(
                    "%s %s %s %s %s %s %s %s chk  \\ %s a=%08X sh=%d"
    ...
    for name, f3, f7 in [("SLLI", 1, 0x00), ("SRLI", 5, 0x00), ("SRAI", 5, 0x20)]:
        for a in SHIFT_VALS:
            for sh in range(32):
                ...
                wlo, whi = lohi(w)
                out.append(
                    "%s %s 0000 0000 %s %s %s %s chk  \\ %s a=%08X sh=%d"
    ...
    for name, f3 in I_OPS:
        for a in EDGES:
            for imm in IMMS:
                ...
                wlo, whi = lohi(w)
                out.append(
                    "%s %s 0000 0000 %s %s %s %s chk  \\ %s a=%08X imm=%03X"
...
def gen_branches(out):
    out.append(
        "\\ === branches BEQ/BNE/BLT/BGE/BLTU/BGEU: taken -> pc+off, else pc+4 ==="
    )
    pc = 0x100
    pairs = [
        (0x5, 0x5),
    ...
    offs = [
        ...
        0xFFE,
        -0x1000,
    ]  # exercise the scrambled B-immediate too
    for name, f3 in [
        ("BEQ", 0),
        ("BNE", 1),
        ("BLT", 4),
        ...
        for off in offs:
            ...
            for a, b in pairs:
                ...
                tlo, thi = lohi(tgt)
                out.append(
                    "%s %s 01 seedr  %s %s 02 seedr  %s %s setpc"
                    % (alo, ahi, blo, bhi, plo, phi)
                )
                out.append(
                    "%s %s brun  %s %s chkpc  \\ %s a=%08X b=%08X off=%d"
...
def gen_jal(out):
    out.append(
        "\\ === JAL: RVPC=target, link x3=PC+4 (checked by chkj against 0x204) ==="
    )
    pc = 0x200
    for off in [
        0x4,
        ...
        tlo, thi = lohi(tgt)
        out.append(
            "%s %s setpc  %s %s jrun  %s %s chkj  \\ JAL off=%d"
...
def gen_jalr(out):
    ...
    for base in [0x400, 0x401, 0x1000, 0x0, 0xFFFE, 0x12344]:
        for imm in [
            ...
            1,
            3,
        ]:  # odd base/imm exercise the &~1 alignment
            ...
            out.append(
                "%s %s setpc  %s %s jrun  %s %s chkj  \\ JALR base=%08X imm=%d"
...
def gen_upper(out):
    out.append(
        "\\ === upper immediates LUI (rd=imm20<<12) / AUIPC (rd=pc+imm20<<12) ==="
    )
    ...
    for imm in imm20s:
        for pc in [0x2000, 0x00001234, 0xFFFF0000]:
            ...
            plo, phi = lohi(pc)
            out.append(
                "%s %s setpc  %s %s runi  %s %s eqx3  \\ AUIPC 0x%05X pc=%08X"
...
def harness():
    ...
    for line in open(spec):
        ...
        if in_def:
            ...
            if (
                ";" in code.split()
            ):  # `;` as a token (may trail a `( -- )` stack comment)
                in_def = False
    if defs < 9:
        sys.exit(
            "error: expected 9 harness defs in rv32i-spec.fth, found %d -- did it change?"
            % defs
        )
    return lines
...
def verify_model():
    ...
    for line in open(spec):
        code = line.split("\\", 1)[0]
        stripped = code.strip()
        if not in_def and stripped.startswith(
            ":"
        ):  # skip whole `: ... ;` definition blocks
            in_def = True
        if in_def:
            if ";" in code.split():
                in_def = False
            continue
        if (
            stripped in ("hex", "decimal", "bye", "decimal cr", "")
            or "variable" in code
        ):
            continue
        for t in code.split():
            try:
                stack.append(int(t, 16))
                continue
            except ValueError:
                pass
            if t == "seedr":
                ...
                alo = stack.pop()
                got = _model_result(
                    (ih << 16) | il, (ahi << 16) | alo, (bhi << 16) | blo
                )
                want = (whi << 16) | wlo
                if got == want:
                    ok += 1
                    ...
                    bad += 1
                    fail("chk got=%X want=%X" % (got, want), line)
                ...
                if op == 0x63:  # branch
                    ...
                    last = {
                        "pc": jal_target(pc, dec_j_imm(word)),
                        "link": (pc + 4) & MASK,
                    }
                elif op == 0x67:  # JALR (rs1 from the word)
                    base = reg[(word >> 15) & 0x1F]
                    last = {
                        "pc": jalr_target(base, dec_i_imm(word)),
                        "link": (pc + 4) & MASK,
                    }
                elif op == 0x37:  # LUI
                ...
                if last.get("pc") == want:
                    ...
                    bad += 1
                    fail("chkpc got=%X want=%X" % (last.get("pc"), want), line)
            elif t == "chkj":  # ( pclo pchi -- ); link asserted ==0x204
                phi = stack.pop()
                plo = stack.pop()
                want = (phi << 16) | plo
                if last.get("pc") == want and last.get("link") == 0x204:
                    ok += 1
                else:
                    bad += 1
                    fail(
                        "chkj got=%X/%X want=%X/204"
                        % (last.get("pc"), last.get("link"), want),
                        line,
                    )
            elif t == "eqx3":
                whi = stack.pop()
                ...
                if last.get("skip"):
                    skip += 1
                elif last.get("x3") == want:
                    ok += 1
                else:
                    bad += 1
                    fail("eqx3 got=%X want=%X" % (last.get("x3"), want), line)
            # else: an unknown token (rvstep internals etc.) -- ignore
    print(
        "model vs rv32i-spec.fth anchors: %d ok, %d mismatched, %d memory-skipped"
        % (ok, bad, skip),
        file=sys.stderr,
    )
    return bad == 0
...
def main(argv):
    ...
    out = [
        "\\ RV32I conformance suite -- GENERATED by scripts/rv32i-conformance.py; do not edit.",
