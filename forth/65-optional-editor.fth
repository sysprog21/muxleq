...
\ to that grid and the block buffer IS the file. Normal mode: hjkl motion, i
\ insert (overwrite+advance), x delete (shift row left). Colon commands: :w save,
\ :n/:p next/previous block, :z blank block, :x save+load (compile) the block on
\ exit, :q quit; ZZ also quits. Reuses page/at-xy/block/update/flush/load/.emit;
...
:s vdel                                ( -- : delete char, shift row left, blank col 63 )
  ...
  scr @ block vy @ [ $6 ] literal lshift + [ $3F ] literal + bl swap c!
  update ;s
...
:s vkey                                ( c -- f : handle one normal key, nonzero quits )
  ...
  dup [char] : = if drop vcmd vcolon dup if exit then drop #0 exit then
